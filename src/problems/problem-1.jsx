...
function Problem1() {
  ...
  // DONE: Create `pieces` — an array of objects with id, label, and color. Use pieceIds.map((id) => ({ ... })) and COLORS[id].
  const pieces = pieceIds.map((id) => ({ id, label: `Piece ${id}`, color: COLORS[id] }));
  return (
    <section className="problem-view p-6">
      ...
      {/* DONE: Render a <ul> with one <li> per piece. Use pieces.map((piece) => <li key={piece.id}>...</li>). Show piece.label; optionally use style={{ color: piece.color }}. */}
